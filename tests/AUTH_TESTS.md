...
> **Важливо:** перед запуском тестів на пристрої має бути **вимкнена** автентифікація. Якщо auth увімкнено — тести не зможуть виконати повний цикл перевірки і будуть автоматично пропущені з відповідним повідомленням.
...
Перед запуском тестів виконується пінг пристрою та перевірка стану auth. Якщо пристрій недоступний або auth увімкнено — весь набір тестів пропускається.
...
- **`ensure_auth_disabled`** — якщо auth увімкнено, вимикає його перед класом і вмикає знову після.
- **`ensure_auth_enabled`** — якщо auth вимкнено, встановлює тестові облікові дані і вмикає auth перед класом, вимикає після.

Тести безпечно запускати за будь-якого початкового стану пристрою — він завжди повертається до вихідного стану.
