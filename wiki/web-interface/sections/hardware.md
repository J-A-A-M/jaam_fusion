...
- **Апаратне забезпечення → Максимальна гучність DFPlayer (апаратна)** — з’являється лише коли задано обидва DF Player піни й обрано джерело PRO/Mini

Ці піни мають сенс лише тоді, коли в секції **"Звук"** вибрано відповідне джерело (Buzzer, DF Player PRO або DF Player Mini). DF Player (RX)/(TX) — спільні піни для PRO і Mini, обирається лише один бекенд одночасно через **Звук → Джерело звуку**.

!!! note
    Прибрали пін, від якого залежить активне джерело звуку? Веб‑інтерфейс сам поверне **Звук → Джерело звуку** в **"Вимкнено"**.
