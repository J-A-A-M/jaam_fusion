...
String JaamWeb::getHtmlTemplate() {
    ...
    html += "        {key: 'air', label: 'Повітряна тривога'},";
    html += "        {key: 'artillery', label: 'Артобстріл'},";
    html += "        {key: 'urban', label: 'Вуличні бої'},";
    html += "        {key: 'chemical', label: 'Хімічна'},";
    html += "        {key: 'nuclear', label: 'Ядерна'},";
    html += "        {key: 'drones', label: 'Дрони'},";
    html += "        {key: 'missiles', label: 'Ракети'},";
    html += "        {key: 'kab', label: 'КАБ'},";
    html += "        {key: 'ballistic', label: 'Балістична'},";
    html += "        {key: 'explosion', label: 'Вибухи'}";
    html += "      ];";
    html += "      data.regions.forEach(region => {";
    html += "        let icons = '';";
    html += "        alertTypes.forEach(type => {";
    html += "          if (region.alerts && region.alerts[type.key]) {";
    html += "            icons += `<span title='${type.label}'><svg class=\"alert-detail-icon\" viewBox=\"0 0 24 24\"><path d=\"M13,14H11V10H13M13,18H11V16H13M1,21H23L12,2L1,21Z\"/></svg></span>`;";
    html += "          }";
    html += "        });";
